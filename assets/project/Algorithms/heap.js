...
function createHeapVis(element) {
    ...
    function resetState() {
        ...
        var w = Math.min(770, document.documentElement.clientWidth-30),
    ...
    function pause() {
        paused = true;
        element.select(".pause-label").text("恢复");
        element.select(".icon-pause").attr("style", "display:none");
        element.select(".icon-play").attr("style", "display:display");
    }

    function resume() {
        paused = false;
        element.select(".pause-label").text("暂停");
        element.select(".icon-pause").attr("style", "display:display");
        element.select(".icon-play").attr("style", "display:none");
    }
